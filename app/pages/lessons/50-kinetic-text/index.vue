...
onMounted(() => {
  ...
  /** Pulls the camera back just far enough to frame the whole headline. */
  const fitCameraToText = () => {
    if (!kineticText) {
      return
    }

    const extent = kineticText.bounds.getSize(new THREE.Vector3())
    ...
    // Generous margin: knocked letters travel well past the headline's own box,
    // and clipping them at the frame edge looks like a bug rather than a limit.
    const distance = Math.max(distanceForHeight, distanceForWidth) * 1.28 + extent.z

    // A narrow FOV pushes the camera far back, and the shared 0.1/100 clip
    // planes then either clip a large `size` outright or waste the whole depth
    // buffer on empty space. Scale them with the framing instead.
    camera.near = distance * 0.05
    camera.far = distance * 3
    camera.updateProjectionMatrix()

    // Dead-on. The shadows face the camera now, so nothing needs a tilt to be
    // visible — and a tilt would only splay the resting extrusions open.
